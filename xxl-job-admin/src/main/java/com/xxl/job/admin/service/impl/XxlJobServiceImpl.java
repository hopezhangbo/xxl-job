...
public class XxlJobServiceImpl implements XxlJobService {
	...
	public Stack<List<XxlJobInfo>> getParentXXlJob(int id) {
		List<XxlJobInfo> jobs = xxlJobInfoDao.getParentJob(id);
		if (jobs == null || jobs.isEmpty())
			return null;
		Stack<List<XxlJobInfo>> stackList = new Stack<List<XxlJobInfo>>();
		setParent(stackList, jobs);
		return stackList;
	}

	private void setParent(Stack<List<XxlJobInfo>> stackList, List<XxlJobInfo> jobs) {
		if (jobs == null || jobs.isEmpty())
			return;
		stackList.push(jobs);
		List<XxlJobInfo> jobList = xxlJobInfoDao.getBatchParentJob(jobs);
		setParent(stackList, jobList);
	}
